// This is the app code for the Bristlemouth dev board
// It uses the user_code.h and makefile from Bristlemouth's serial payload example:
// https://github.com/bristlemouth/bm_protocol/tree/develop/src/apps/bm_devkit/serial_payload_example
...
#define RPI_BUFFER_SIZE 311
static uint8_t current_rpi_buffer[RPI_BUFFER_SIZE];
static size_t current_rpi_buffer_count = 0;
...
void loop(void) {
  ...
  if (readingBytesTimer == -1 && PLUART::byteAvailable()) {
    ...
    // Check for available bytes and read them into the buffer
  while (PLUART::byteAvailable()) {
    if (current_rpi_buffer_count < RPI_BUFFER_SIZE) {
      uint8_t byte_read = PLUART::readByte();
      current_rpi_buffer[current_rpi_buffer_count++] = byte_read;
    } else {
      // Buffer full, handle overflow or flush here
      // TODO empty it so that the last (incomplete) buffer doesnt have old values
    }
    // Reset the timer since we've just read a byte
    readingBytesTimer = (int64_t)((u_int32_t)uptimeGetMs());
  }
  // No more bytes available and we have data in the buffer
  if (!PLUART::byteAvailable() && current_rpi_buffer_count > 0 &&
      (u_int32_t)uptimeGetMs() - (u_int32_t)readingBytesTimer >= BYTES_CLUSTER_MS) {
    // Print the buffered bytes all at once
    for (size_t i = 0; i < current_rpi_buffer_count; i++) {
      printf("%02X ", current_rpi_buffer[i]);
      bm_fprintf(0, "rpi_bytes.log", "%02X\n", current_rpi_buffer[i]);
    }
    printf("\n");
    // Reset the buffer count after printing
    current_rpi_buffer_count = 0;
    // Reset the timer
    readingBytesTimer = -1;
  }
  ...
  if (PLUART::lineAvailable()) {
    ...
    //uint16_t payload_buffer[] = {0x01, 0x02, 0x03, 0x04, 0x69}; // Example data bytes

    // ELLIE ADDITION: try to tx data via cellular
